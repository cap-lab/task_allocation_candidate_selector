...
def allocate(robot, tasks, allocator_type, dst_dir):
    ...
    allocator = Allocator()
    environment = Environment(robot, tasks, robot['comm'], allocator, dst_dir)
    environment.compile_time_allocate()

def tester(data, test_file, allocator_type, dst_dir):
    allocate(copy.deepcopy(data['robot']), copy.deepcopy(data['task']), allocator_type, dst_dir)

def argument_init():
    import argparse
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('-t', '--test', type=str, default='../input/dynamic_test.yml', help='Test Scenario')
    parser.add_argument('-s', '--src_dir', type=str, default='../input/testcase/', help='Source Directory')
    parser.add_argument('-d', '--dst_dir', type=str, default='../output/', help='Destination Directory')
    parser.add_argument('-a', '--allocator', type=str, default='default', help='Allocator Type')
    return parser.parse_args()

def process(test_name, testcase, process_list, allocator_type, dst_dir):
    lock = Lock()
    with open(args.src_dir + testcase['file'], 'r') as f:
    ...
    if 'change' in testcase.keys():
        ...
        while len(change_set)!=0:
            ...
            if item[2] == len(keys):
                proc = Process(target=tester, args=(data, testcase['file'], allocator_type, dst_dir))
                proc.start()
...
if __name__ == "__main__":
    ...
    for testcase in test['testcase']:
        process(test['name'], testcase, process_list, args.allocator, args.dst_dir)
